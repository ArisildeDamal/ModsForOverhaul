	...
	public class ModelSphereUtil
	{
		...
		public static MeshData GetSphereModelData(float radius, float height, int segments, int rings)
		{
			...
			for (int y = 0; y < rings; y++)
			{
				...
				float phiFloat = yFloat / (float)(rings - 1) * ModelSphereUtil.GetPi();
				for (int x = 0; x < segments; x++)
				{
					float num = (float)x;
					float thetaFloat = num / (float)(segments - 1) * 2f * ModelSphereUtil.GetPi();
			...
			meshData.SetIndices(ModelSphereUtil.CalculateElements(radius, height, segments, rings));
			return meshData;
		}
