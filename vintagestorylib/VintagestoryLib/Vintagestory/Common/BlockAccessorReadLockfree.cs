...
namespace Vintagestory.Common
{
	public class BlockAccessorReadLockfree : IBlockAccessor
	{
		...
		{
			return this.GetBlockId(pos.X, pos.Y, pos.Z, 0);
		}
		...
		{
			return this.worldmap.Blocks[this.GetBlockId(x, y, z, layer)];
		}

		public Block GetBlock(int x, int y, int z, int layer)
		{
			return this.GetBlockRaw(x, y, z, layer);
		}

		public Block GetBlock(BlockPos pos, int layer)
		{
			return this.worldmap.Blocks[this.GetBlockId(pos, layer)];
		}

		public Block GetMostSolidBlock(BlockPos pos)
		{
			return this.GetMostSolidBlock(pos.X, pos.InternalY, pos.Z);
		}
		...
		public Block GetMostSolidBlock(int posX, int posY, int posZ)
		{
			...
			{
				return this.worldmap.Blocks[0];
			}
			IWorldChunk chunk = this.worldmap.GetChunkNonLocking(posX / 32, posY / 32, posZ / 32);
			if (chunk == null)
			{
				return this.worldmap.Blocks[0];
			}
		...
		{
			IWorldChunk chunk = this.GetChunkAtBlockPos(position);
			if (chunk == null)
			{
				return null;
			}
		...
		public int GetBlockId(int posX, int posY, int posZ, int layer)
		{
			...
			if (chunk != null)
			{
				return chunk.MaybeBlocks.GetBlockId(this.worldmap.ChunkSizedIndex3D(posX & 31, posY & 31, posZ & 31), layer);
			}
			return 0;
		}
		...
		public int GetBlockId(BlockPos pos, int layer)
		{
			...
			IWorldChunk chunk = this.worldmap.GetChunkNonLocking(pos.X / 32, pos.Y / 32, pos.Z / 32);
			if (chunk != null)
			{
				return chunk.MaybeBlocks.GetBlockId(this.worldmap.ChunkSizedIndex3D(pos.X & 31, pos.Y & 31, pos.Z & 31), layer);
			}
			return 0;
		}
		...
		public Block GetBlockOrNull(int posX, int posY, int posZ, int layer = 4)
		{
			...
			if (chunk != null)
			{
				return this.worldmap.Blocks[chunk.MaybeBlocks[this.worldmap.ChunkSizedIndex3D(posX & 31, posY & 31, posZ & 31)]];
			}
			return null;
		}
		...
		{
			return this.worldmap.GetChunkNonLocking(chunkX, chunkY, chunkZ);
		}
		...
		{
			return this.worldmap.GetChunkNonLocking(posX / 32, posY / 32, posZ / 32);
		}

		public IWorldChunk GetChunkAtBlockPos(BlockPos pos)
		{
			return this.worldmap.GetChunkNonLocking(pos.X / 32, pos.Y / 32, pos.Z / 32);
		}
		...
		public int GetLightRGBsAsInt(int posX, int posY, int posZ)
		{
			...
			int num2 = (int)worldmap.hueLevels[blockhue];
			int blocks = (int)worldmap.satLevels[blocksat];
		...
		{
			return this.worldmap.GetLightRGBSVec4f(pos.X, pos.Y, pos.Z);
		}
		...
		{
			return this.worldmap.IsValidPos(pos.X, pos.Y, pos.Z);
		}
		...
		{
			int num = GameMath.Clamp(minPos.X, 0, this.worldmap.MapSizeX);
			int miny = GameMath.Clamp(minPos.Y, 0, this.worldmap.MapSizeY);
			int minz = GameMath.Clamp(minPos.Z, 0, this.worldmap.MapSizeZ);
			int maxx = GameMath.Clamp(maxPos.X, 0, this.worldmap.MapSizeX);
			int maxy = GameMath.Clamp(maxPos.Y, 0, this.worldmap.MapSizeY);
			int maxz = GameMath.Clamp(maxPos.Z, 0, this.worldmap.MapSizeZ);
			...
			for (int x = num; x <= maxx; x++)
			{
				for (int y = miny; y <= maxy; y++)
				{
					for (int z = minz; z <= maxz; z++)
					{
						...
						int cz = z / 32;
						IWorldChunk chunk = this.worldmap.GetChunkNonLocking(cx, cy, cz);
						if (chunk != null)
						{
							Block block = this.worldmap.Blocks[chunk.MaybeBlocks[index3d]];
							onBlock(block, x, y, z);
						}
					}
				}
		...
		{
			return this.GetBlockEntity(position) as T;
		}
